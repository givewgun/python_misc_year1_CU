...
def isValidWord(word, hand, wordList):
    ...
    statement = True
    for char in word:
        if char not in list(hand.keys()) or hand_copy[char] <= 0 : 
            statement= False
        else:
            hand_copy[char] -= 1
    return statement and word in wordList
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand_new) >= 0:
        ...
        if word == '.':
            ...
            print('Goodbye! Total scores ' + str(total))
            break
        # Otherwise (the input is not a single period):
        elif not isValidWord(word, hand_new, wordList): # If the word is not valid
